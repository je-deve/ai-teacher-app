...
def draw_smart_table_row(pdf, title, content_points):
    ...
    col_title = 45; col_content = 145; lh = 8; pad = 4
    
    lines = []
    for p in content_points:
        clean = p.strip().replace('-','').replace('•','').strip()
    ...
    y = pdf.get_y()
    pdf.set_fill_color(253,245,230); pdf.rect(155, y, col_title, h, 'FD') # Title Box
    pdf.set_fill_color(255,255,255); pdf.rect(10, y, col_content, h, 'FD') # Content Box
    
    try: pdf.set_font('AmiriB','',13)
    except: pass
    pdf.set_text_color(101,67,33)
    pdf.set_xy(155, y+(h/2)-3); pdf.cell(col_title, 6, ar(title), 0, 0, 'C')
...
def gemini_analyze_audio(path, ref_text, lang="ar"):
    try:
        ...
        if lang == "ar":
            ...
            المطلوب:
            1. قيّم القراءة بدقة واحسب الدرجات.
            2. أضف "التقييم العام" (Overall Result) كنسبة مئوية في جدول الدرجات.

            التنسيق (التزم به بدقة):
            الوعي الصوتي|__/25
            قراءة المقاطع|__/24
            ...
            prompt = f"""
            You are an educational expert. Ref: "{ref_text}"
            
            Task: Evaluate reading and provide scores.
            Include "Overall Score" as a percentage in the score list.
...
def analyze_en():
    ...
    try:
        ...
        # Scores Table (Overall included here)
        if scores_data:
