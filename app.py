...
def draw_level_badge(pdf, level_text, x, y, lang='ar'):
    try:
        ...
        if lang == 'ar':
            ...
            except: pdf.set_font('Amiri', '', font_size)
            level_disp = ar(level)
        else:
...
class ArabicPDF(BasePDF):
    def header(self):
        ...
        try:
            ...
            self.add_font('AmiriB', '', 'Amiri-Bold.ttf')
...
# ================== AI Logic ==================
def gemini_analyze_audio(path, ref_text, grade, lang="ar"):
    try:
        ...
        if lang == "ar":
            prompt = f"""
            أنت معلم صبور. الطالب في الصف: {grade}. النص المرجعي: "{ref_text}"
            
            هام جداً: الطالب طفل صغير، لذا التوقفات البسيطة للتهجئة تعتبر قراءة صحيحة. لا تحسم درجات على البطء طالما النطق سليم.
            
            المهمة: استخرج الدرجات والتقييم بناءً على مستوى الصف {grade}.
            التنسيق (التزم به):
            الوعي الصوتي|__/25
            قراءة المقاطع|__/24
            الكلمات الشائعة|__/20
            الطلاقة القرائية|__ كلمة/دقيقة
            التقييم العام|(عالي، متوسط، ضعيف)
            ...
            prompt = f"""
            Expert Teacher. Student in {grade}. Ref: "{ref_text}"
            
            IMPORTANT: Student is a young child. Slow decoding (spelling out letters) is EXCELLENT. Do not penalize for slow pace or pauses between words. Focus on accuracy.
            ...
            Fluency|__ wpm
            Intonation|__/15
            Overall Level|(High, Medium, or Low)
            SCORES_END
...
def analyze_ar():
    ...
    grade = (request.form.get('grade') or 'الأول الابتدائي').strip()
    ...
    try:
        ...
        with ThreadPoolExecutor(1) as ex:
            ai_text = ex.submit(gemini_analyze_audio, fname, ref_text, grade, "ar").result(GEMINI_TIMEOUT_SECONDS)
        
        if "GEMINI_ERROR" in ai_text: return f"Error: {ai_text}", 500

        table_data = []; overall_level = "متوسط"; sections = {"تحليل الأخطاء":[],"مؤشرات الأداء":[],"التوصيات":[]}; curr_sec = None
        for line in ai_text.split('\n'):
            ...
            if '|' in clean:
                ...
                if len(parts) >= 2:
                    ...
                    if "التقييم العام" in k: overall_level = v
                    elif any(x in k for x in ["الوعي", "المقاطع", "الكلمات", "الطلاقة"]): table_data.append((k,v))
        ...
        pdf = ArabicPDF(); pdf.add_page()
        draw_level_badge(pdf, overall_level, x=35, y=pdf.get_y()+8, lang='ar')

        pdf.set_fill_color(240,240,240); pdf.set_text_color(101,67,33); pdf.set_xy(65, pdf.get_y())
        pdf.cell(45,10,ar("التاريخ"),1,0,'C',1)
        pdf.cell(45,10,ar("الصف"),1,0,'C',1)
        pdf.cell(50,10,ar("اسم الطالب"),1,1,'C',1)
        pdf.set_xy(65, pdf.get_y())
        pdf.set_fill_color(255,255,255)
        pdf.cell(45,10,date.today().strftime("%Y/%m/%d"),1,0,'C',1)
        pdf.cell(45,10,ar(grade),1,0,'C',1)
        pdf.cell(50,10,ar(name),1,1,'C',1)
        pdf.ln(15)

        if table_data:
            pdf.set_fill_color(184,134,11); pdf.set_text_color(255,255,255)
            pdf.cell(60,10,ar("الدرجة"),1,0,'C',1); pdf.cell(130,10,ar("المعيار"),1,1,'C',1)
            pdf.set_text_color(0,0,0); fill=False
            for k,v in table_data:
                pdf.set_fill_color(245,245,245) if fill else pdf.set_fill_color(255,255,255)
                pdf.cell(60,10,ar(v),1,0,'C',fill); pdf.cell(130,10,ar(k),1,1,'R',fill)
                fill=not fill
            pdf.ln(10)

        for title, content in sections.items(): draw_dynamic_row(pdf, title, content, 'ar')

        out_name = f"Rep_AR_{uuid.uuid4().hex[:6]}.pdf"
        pdf.output(out_name)
...
def analyze_en():
    ...
    grade = (request.form.get('grade') or 'Grade 1').strip()
    ref_text = (request.form.get('ref_text') or '').strip()
    f = request.files.get('audio_upload') or request.files.get('audio_record')
    if not f: return "No audio", 400
    ...
    try:
        f.save(fname)
        with ThreadPoolExecutor(1) as ex:
            ai_text = ex.submit(gemini_analyze_audio, fname, ref_text, grade, "en").result(GEMINI_TIMEOUT_SECONDS)

        if "GEMINI_ERROR" in ai_text: return f"Gemini Error: {ai_text}", 500

        scores_data = []; overall_level = "Medium"; notes = {"Error Analysis":[],"Performance Overview":[],"Recommendations":[]}; curr_note = None; in_scores = False; in_notes = False
        for line in ai_text.split('\n'):
            ...
            if in_scores and '|' in clean:
                parts = clean.split('|')
                if len(parts) >= 2:
                    if "Overall Level" in parts[0]: overall_level = parts[1].strip()
                    else: scores_data.append((parts[0].strip(), parts[1].strip()))
            if in_notes:
                ...
                elif curr_note and clean.startswith('-'): notes[curr_note].append(clean)

        pdf = EnglishPDF(); pdf.add_page()
        pdf.set_fill_color(240,240,240); pdf.set_draw_color(184,134,11); pdf.set_text_color(101,67,33)
        pdf.cell(50,10,"Date",1,0,'C',1); pdf.cell(50,10,"Grade",1,0,'C',1); pdf.cell(70,10,"Student Name",1,1,'C',1)
        pdf.set_xy(10, pdf.get_y()+10)
        pdf.set_fill_color(255,255,255)
        pdf.cell(50,10,date.today().strftime("%Y/%m/%d"),1,0,'C',1); pdf.cell(50,10,clean_en(grade),1,0,'C',1); pdf.cell(70,10,clean_en(name),1,1,'C',1)
        draw_level_badge(pdf, overall_level, x=185, y=pdf.get_y()-5, lang='en')
        pdf.ln(18)

        if scores_data:
            pdf.set_text_color(101,67,33); pdf.cell(0,10,"Assessment Scores:",0,1,'L'); pdf.ln(2)
            pdf.set_fill_color(184,134,11); pdf.set_text_color(255,255,255)
            pdf.cell(130,10,"Criteria",1,0,'L',1); pdf.cell(60,10,"Score",1,1,'C',1)
            pdf.set_text_color(0,0,0); fill=False
            for c,s in scores_data:
                pdf.set_fill_color(245,245,245) if fill else pdf.set_fill_color(255,255,255)
                pdf.cell(130,10,"  "+clean_en(c),1,0,'L',fill); pdf.cell(60,10,clean_en(s),1,1,'C',fill)
                fill=not fill
            pdf.ln(12)

        for title, content in notes.items(): draw_dynamic_row(pdf, title, content, 'en')
